...
export default function QuickViewModal({ product, isOpen, onClose }) {
    ...
    // SINGLE initialization useEffect - FIXED: Force all selections to false
    useEffect(() => {
        if (product) {
            ...
            if (product.customizations) {
                Object.entries(product.customizations).forEach(([categoryId, options]) => {
                    initialCustomizations[categoryId] = options.map(opt => ({
                        ...
                        selected: false, // FORCE ALL TO FALSE - NO PRE-SELECTION
            ...
            console.log('Initialized customizations (all unselected):', initialCustomizations);
        }
    }, [product]);

    // SINGLE price calculation useEffect (removed duplicate)
    useEffect(() => {
        if (!product) return;
    ...
    // Update main image when customization is selected
    useEffect(() => {
        if (!product) return;
    ...
    const handleCustomizationToggle = (categoryId, optionId) => {
        setSelectedCustomizations(prev => {
            ...
            if (existing) {
                ...
                // Select the new option (only one per category)
